...
class ModalGallery2 extends Component {
  ...
  render() {
    const { dataPage } = this.props;
    return (
      <div className = 'ModalGallery2 animated fadeIn'>
        <div className = 'showModal'>
          <div className = 'itemGallery mB-1 d-Flex j-C aI-C'>
            {
              dataPage.length > 1 ?
              <button className = { 'buttonSlide d-Flex j-Ar aI-C mB-025 mL-7' + (this.state.slide === 1 ? 'disabled' : 'pulse') } id = 'btnAnt' onClick = { this.mSlides }>
            ...
            {
              dataPage.length > 1 ?
              <button className = { 'buttonSlide d-Flex j-Ar aI-C aS-C pT-7' + (this.state.slide === dataPage.length ? 'disabled' : 'pulse') } id = 'btnSig' onClick = { this.mSlides }>
                <span className = 'fa-layers fa-fw iconButton' >
                  <FontAwesomeIcon icon="circle" />
